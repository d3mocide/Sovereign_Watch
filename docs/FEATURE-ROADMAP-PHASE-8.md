...
1. **Fetches TLE data** from Celestrak public endpoints (no auth required) for the following
   categories (use the GP data JSON API, not legacy TLE text format):
   - GPS: `https://celestrak.org/SOCRATES/query.php` — actually use the GP endpoint:
     `https://celestrak.org/SOCRATES/query.php` is wrong, use:
     `https://celestrak.org/satcat/tle.php?CATNR=<id>` for single objects, OR
     the categorised JSON GP feeds:
     - GPS: `https://celestrak.org/GPS/almanac.php?URL=ops-almanac.semaine.yuma.txt`
     Actually, use the correct Celestrak GP JSON API:
     `https://celestrak.org/SOCRATES/query.php` — No. Use:
     `https://celestrak.org/satcat/records.php?CATNR={id}&FORMAT=json`
     The correct bulk TLE-JSON feeds are:
     `https://celestrak.org/SOCRATES/query.php?FORMAT=json` — No.

     **IMPORTANT**: Fetch the correct Celestrak category TLE files:
     - GPS: `https://celestrak.org/supplemental/gps-ops.txt`
     - Weather: Use the standard TLE group `weather`:
       `https://celestrak.org/SOCRATES/query.php?FORMAT=TLE` — still wrong.

     Use Celestrak's **REST API for supplemental TLE data**:
     `GET https://celestrak.org/SOCRATES/query.php?CATNR={norad_id}&FORMAT=json`

     ACTUALLY — Ignore the above confusion. Use the simple, well-documented approach:
     Fetch standard Celestrak TLE group files in 3-line TLE text format:
     - `https://celestrak.org/supplemental/gps-ops.txt` (GPS)
     - `https://celestrak.org/supplemental/galileo.txt` (Galileo)
     - `https://celestrak.org/supplemental/glonass-ops.txt` (GLONASS)
     - `https://celestrak.org/supplemental/beidou.txt` (BeiDou)
     - `https://celestrak.org/TLE/catalog/noaa.txt` (Weather)
     - `https://celestrak.org/TLE/catalog/active.txt` (Active Satellites — broad category)
     Parse 3-line TLE blocks: name line + TLE line 1 + TLE line 2.

2. **Propagates positions** using the `sgp4` Python library (`from sgp4.api import Satrec, jday`).
...
3. **Publishes TAK Protobuf** messages to Kafka topic `orbital_raw`.
   - TAK type: `a-s-K`
   - uid: `SAT-{NORAD_ID}`
   - callsign: satellite name from TLE
   ...
   - detail: JSON string containing `{"norad_id": int, "category": str, "period_min": float, "inclination_deg": float}`

4. **Refreshes TLE data** every **6 hours** (Celestrak updates TLEs twice daily).
...
7. **Add `orbital_raw` topic** to the Redpanda init script / docker-compose topic creation
   if one exists, or document the manual `rpk topic create orbital_raw --partitions 4` command.
