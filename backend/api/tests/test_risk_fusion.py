...
async def cleanup_data(conn):
    ...
    await conn.execute(
        "DELETE FROM tracks WHERE entity_id LIKE 'icao-test-fusion-air%'"
    )
    await conn.execute(
        "DELETE FROM tracks WHERE entity_id LIKE 'mmsi-test-fusion-sea%'"
    )
    ...
    async with db.pool.acquire() as conn:
        ...
        anomalies = [
            AnomalyMetric(
                ...
                score=0.8,
                affected_uids=["test-air"],
                description="Aviation",
            ),
            AnomalyMetric(
                metric_type="satellite_signal_loss",
                score=0.8,
                affected_uids=["test-sat"],
                description="Orbital",
            ),
        ]

        # Baseline (single domain)
        score_single = detector.compute_risk_score(
            0.0, 0.0, 0.0, context_anomalies=anomalies[:1]
        )
        # Dual domain (convergence)
        score_dual = detector.compute_risk_score(
            0.0, 0.0, 0.0, context_anomalies=anomalies
        )
        ...
        assert abs(score_dual - expected_boosted) < 0.01, (
            f"Convergence boost not applied correctly! Expected {expected_boosted}, got {score_dual}"
        )
        ...
        target_cell_data = next(
            (c for c in risk_result.cells if c.cell == TEST_CELL), None
        )
        ...
        if target_cell_data:
            ...
            assert target_cell_data.sentiment == 1.0, (
                "GDELT sentiment should be 1.0 for conflict"
            )
            print(" VERIFIED: GDELT Sentiment risk mapped correctly.")
        else:
            print(
                " WARNING: Target cell not found in risk output (may be hidden by other high-density cells)."
            )
        ...
        if decay_cell and target_cell_data:
            ...
            assert decay_cell.density < target_cell_data.density, (
                "Density score did not decay!"
            )
