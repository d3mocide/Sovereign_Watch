    ...
    for sat in satellites:
        ...
        for idx, step_idx in enumerate(valid_indices):
            ...
            if el >= min_elevation:
                ...
                t = now_no_ms + timedelta(seconds=int(step_idx) * step_seconds)
                # ⚡ Bolt: isoformat() is ~2x faster than t.strftime("%Y-%m-%dT%H:%M:%SZ")
                point = {
...
async def get_groundtrack(
    ...
    for idx, i in enumerate(valid_indices):
        t = now_no_ms + timedelta(seconds=i * step_seconds)
        # ⚡ Bolt: isoformat() is ~2x faster than t.strftime("%Y-%m-%dT%H:%M:%SZ")
        points.append(
            {
                "t": t.isoformat().replace("+00:00", "Z"),
