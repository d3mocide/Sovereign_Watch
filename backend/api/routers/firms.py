...
async def query_dark_vessel_features(
    conn,
    *,
    min_lat: float = -90.0,
    max_lat: float = 90.0,
    ...
    match_radius_nm: float = _DEFAULT_MATCH_RADIUS_NM,
    min_frp: float = _DEFAULT_MIN_FRP,
    ais_window_h: float = _DEFAULT_AIS_WINDOW_H,
    min_risk_score: float = 0.0,
) -> list[dict]:
    """Cross-reference FIRMS hotspots against AIS tracks and return dark-vessel
    candidate GeoJSON features.

    For each qualifying hotspot the nearest AIS-type track within
    *match_radius_nm* during the ±ais_window_h window is located; hotspots
    with no nearby AIS track (or one only at the edge of the radius) are
    dark-vessel candidates.  Shared by the /api/firms/dark-vessels endpoint
    and the sea-domain analyst (/api/ai_router/analyze/sea).
    """
...
async def get_dark_vessels(
    ...
    try:
        async with db.pool.acquire() as conn:
            features = await query_dark_vessel_features(
                conn,
                min_lat=min_lat,
                max_lat=max_lat,
                min_lon=min_lon,
