...
def _headline_mentions_mission_country(
    event_text: object, mission_country_code: str | None
) -> bool:
    normalized_text = _normalize_text(event_text)
    if not normalized_text:
        return False
    return any(
        term in normalized_text for term in _mission_country_terms(mission_country_code)
    )
...
def _support_matches_need_mission_relation(
    *,
    ...
    headline_mentions_mission = _headline_mentions_mission_country(
        event_text, mission_country_code
    )
    return {
        "mission_relation_country_codes": mission_relation,
        "headline_mentions_mission": headline_mentions_mission,
        "support_relation_confirmed": bool(mission_relation)
        or headline_mentions_mission,
    }


def expand_country_neighbors(
    country_code: str | None, *, max_depth: int = 1
) -> Set[str]:
    if not country_code:
        return set()
    if max_depth < 0:
...
def build_experimental_country_sets(
    ...
    mission_and_first_order = expand_country_neighbors(
        mission_country_code, max_depth=1
    )
    mission_and_second_order = expand_country_neighbors(
        mission_country_code, max_depth=2
    )
    alliance_support = set(
        EXPERIMENTAL_ALLIANCE_SUPPORT_COUNTRIES.get(mission_country_code or "", set())
    )
    basing_support = set(
        EXPERIMENTAL_BASING_SUPPORT_COUNTRIES.get(mission_country_code or "", set())
    )
...
def evaluate_experimental_country_matches(
    ...
    *,
    ...
    first_order_matches = sorted(
        actor_country_codes & country_sets.mission_and_first_order
    )
    second_order_matches = sorted(
        actor_country_codes & country_sets.mission_and_second_order
    )
    ...
    second_order_only_matches = [
        code
        for code in second_order_matches
        if code not in country_sets.mission_and_first_order
    ]
    ...
    return {
        ...
        **(
            ...
            if alliance_matches or basing_matches or second_order_only_matches
            else {}
        ),
    }
...
async def fetch_linkage_audit(
    ...
    *,
    ...
    return {
        ...
        "country_sets": {
            "second_order": sorted(
                country_sets.mission_and_second_order
                - country_sets.mission_and_first_order
            ),
