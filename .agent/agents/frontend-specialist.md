...
- **Surfaces**: Translucent panels (glassmorphism via `bg-black/30 backdrop-blur-md shadow-[inset_0_1px_1px_rgba(255,255,255,0.05)]` or `bg-tactical-panel/60 backdrop-blur-xl`) combined with subtle borders (`tactical-border` -> `#1e1e1e` or `border-white/10`).
...
- **Map-Centric**: The layout is built _around_ and _over_ the interactive Deck.gl/Mapbox canvas. Overlays must not block critical map interaction.
