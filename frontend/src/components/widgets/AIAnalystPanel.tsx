...
const AnalysisFormatter: React.FC<{ text: string; isStreaming: boolean; accentColor: string }> = ({ text, isStreaming, accentColor }) => {
  ...
  // 1. Pre-process to handle generic headers and forced line breaks
  const processed = text
    // Normalize headers: capture bold blocks that look like titles
    // - Contains 'Section', ':', or '&'
    // - Follows punctuation or starts the line
    .replace(/\*\*\s*([^*]*?(?:Section|:|&|Analysis|Verification|Indicators|Assessment)[^*]*?)\s*\*\*/gi, '\n\n**$1**\n')
    // Ensure that any bold block starting after a sentence break gets a newline to be treated as a title
    .replace(/([.!?])\s*(\*\*)/g, '$1\n\n$2')
    // --- Post-Processing Cleanup for LLM Spacing Artifacts ---
    .replace(/\s+([,.!?;:])/g, '$1') // Collapse spaces before punctuation
    .replace(/(\w)\s*['â€™'']\s*(\w)/g, "$1'$2") // Fix "it ' s" or "it 's" style spacing artifacts
    .replace(/(\s'|'\s)/g, (m) => m.trim()) // Fallback for other quote cases
    .replace(/\(\s+/g, '(') // Left paren: "( " -> "("
    .replace(/\s+\)/g, ')') // Right paren: " )" -> ")"
    .trim();

  // 2. Split into lines and render
