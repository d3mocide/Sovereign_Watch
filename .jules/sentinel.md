...
**Learning:** Returning unhandled or low-level internal error details to users can leak stack traces, implementation details, or sensitive system state, violating the principle of failing securely.
**Prevention:** Catch exceptions, log the detailed error securely on the server (using `logger.warning` or `logger.error`), and return only a sanitized, generic error message (like 'Malformed TLE' or 'Internal server error') in the HTTP response.
## 2025-02-28 - SSRF Vulnerability in News Feed Article Extraction via HTTPX Connection Pooling
**Vulnerability:** Even when implementing a custom `AsyncHTTPTransport` to protect against SSRF (by resolving IPs to prevent DNS rebinding), if `request.url` is rewritten to an IP address, `httpx` keys its connection pool by that IP instead of the original hostname. This causes `_fetch_feeds` to mistakenly reuse the same TLS connection for different hostnames that share an IP (e.g. Cloudflare or Fastly), leading to 421 Misdirected Request errors or TLS mismatch failures.
**Learning:** Overriding `request.url.host` with an IP address in a transport intercepts the connection successfully but breaks connection pooling for shared hosts, creating stability bugs in concurrent or looping request workflows.
**Prevention:** In functions like `_fetch_feeds` that iterate through multiple URLs using a single `AsyncClient`, instantiate a separate client per request or disable connection pooling. Alternatively, a more advanced `httpx.AsyncHTTPTransport` override would need to manage socket resolution deeper in the stack without modifying `request.url.host`.
