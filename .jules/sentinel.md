...
## 2025-04-16 - SSRF Vulnerability via URL Fetching
**Vulnerability:** The `/api/news/article` endpoint fetched external URLs directly using `httpx` based on user input, and the original blacklist check only blocked literal "localhost" or loopback IPs in the URL string, failing to account for URLs resolving to private or internal IP spaces.
**Learning:** Checking the parsed hostname against a string blacklist is insufficient because DNS records can map seemingly innocuous domains to internal IP addresses (e.g. `192.168.x.x`, `10.x.x.x`), leading to Server-Side Request Forgery (SSRF) which can expose internal services.
**Prevention:** Before performing external network requests with user-supplied URLs, use `asyncio.get_running_loop().getaddrinfo()` to explicitly resolve the hostname to IP addresses, and then iterate through the addresses to assert they are safe using `ipaddress` properties like `is_private`, `is_loopback`, `is_multicast`, or `is_unspecified`. Note that a full mitigation must also bind the resolved IP to prevent DNS rebinding TOCTOU attacks.
