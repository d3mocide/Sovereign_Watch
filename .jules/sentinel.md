...
## 2026-04-18 - SSRF Vulnerability in Article Fetching
**Vulnerability:** Server-Side Request Forgery (SSRF)
**Learning:** `backend/api/routers/news.py` implemented an incomplete SSRF protection in `get_article_content` by only blocking the strings 'localhost', '127.0.0.1' and '::1'. This leaves the application vulnerable to requesting other private subnets (e.g. AWS Instance Metadata Service at `169.254.169.254`) and vulnerable to DNS Rebinding attacks where an attacker provides a public domain that resolves to a private IP.
**Prevention:** When fetching external URLs (e.g., using `httpx`), always resolve the hostname asynchronously using `asyncio.get_running_loop().getaddrinfo()` and verify the resulting IP addresses are not private, loopback, multicast, or unspecified (using the `ipaddress` module) before making the request. Check the host against `ipaddress` to avoid unnecessary DNS lookups for bare IPs.
## 2026-04-18 - TOCTOU SSRF Vulnerability in Article Fetching
**Vulnerability:** DNS Rebinding (TOCTOU SSRF)
**Learning:** When performing SSRF protection by resolving a DNS hostname to check if its IP is private, passing the original hostname to the HTTP client afterwards creates a Time-of-Check to Time-of-Use (TOCTOU) vulnerability. The HTTP client will resolve the DNS again, and an attacker can change the DNS record to point to a private IP in between the check and the HTTP request, bypassing the protection.
**Prevention:** To prevent DNS rebinding attacks, always connect directly to the validated safe IP address instead of relying on the HTTP client to perform a second DNS lookup. You can preserve the original `Host` header in the HTTP request so the remote server still sees the expected domain name.
