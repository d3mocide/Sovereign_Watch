...
## 2026-05-26 - Native duration calculations in high-frequency loops
**Learning:** In performance-critical paths like orbital pass prediction, storing formatted strings only to parse them again with `datetime.strptime` to calculate the duration adds unnecessary overhead.
**Action:** Calculate pass durations natively using raw time offsets (e.g., integer seconds) and only format timestamps when pushing final payload entries.
