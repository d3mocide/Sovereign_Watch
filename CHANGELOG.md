...
- **Backend FIRMS Router Regression Coverage**: Added targeted backend API tests for global FIRMS responses, CSV normalization, polygonal land-mask extraction, and dark-vessel land-mask query parameters.
- **FIRMS Poller Source Health Visibility**: Added per-source FIRMS ingest status reporting so operations surfaces can show which upstream VIIRS feeds are healthy, empty, or failing.
...
- **FIRMS Startup Default**: `showFIRMS` now defaults to `false` while `showDarkVessels` remains enabled, reducing initial thermal noise without hiding anomaly coverage.
- **FIRMS Scope Contract**: FIRMS and dark-vessel frontend fetches now always request explicit world bounds, and the obsolete FIRMS global toggle plus mission-bbox branching were removed.
- **Global FIRMS Ingest Contract**: The Space Pulse poller now ingests the NASA FIRMS world endpoint by default, and the backend treats the primary hotspot cache as global instead of splitting between mission and world modes.
...
- **FIRMS Control Wiring**: Fixed the FIRMS thermal subfilter so the visible control actually toggles the thermal layer and restores to the new off-by-default state.
- **FIRMS Poller Blind Spot**: The operations health view now reports FIRMS as a monitored poller and exposes source-level feed summaries instead of requiring manual log inspection during upstream drift.
